function [f,J,Q] = spm_fx_cmc_2017_constG(x,u,P,M)
% state equations for a neural mass model (canonical microcircuit) - 
% as used in Shaw et al (2017) Neuroimage (sp->dp replaces sp->ss)
%
...
if isfield(M,'cmcj')
    ...
    j = [7 4 12 9  11 6   5 8   1 3 10 13]; 
end
% for i = 1:size(P.G,2)                         % ORIGINAL
%     G(:,j(i)) = G(:,j(i)).*exp(P.G(:,i));
% end
for i = 1:size(P.G,2)                           
    if i == 1
        G(:,j(i)) = G(:,j(i)).*exp(P.G(:,i));   % EXCEPT for sup pyr GAIN (i=1)...
    else
        G(:,j(i)) = G(:,j(i)).*exp(P.G(1,i));   % ...ESTIMATED Gs ARE CONSTANT ACROSS AREAS
%         G(3:6,j(i)) = G(2,j(i));            
    end
end
...
%--------------------------------------------------------------------------
u      =   A{1}*S(:,3) + U;
u      = - G(:,1).*S(:,1) - G(:,3).*S(:,5) + u;     % removed - G(:,2).*S(:,3) 
...
%--------------------------------------------------------------------------
u      =   A{2}*S(:,3);
u      =   G(:,13).*S(:,3) - G(:,10).*S(:,7) - G(:,9).*S(:,5) + u; % added + G(:,13).*S(:,3)
